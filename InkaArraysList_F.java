...
public class InkaArraysList {
    ...
    private static void CorreosContrase() {
        NameUser.add("Administrador");NameUser.add("cliente");
        EmailRegister.add("Admin@com");EmailRegister.add("cliente@com");
    ...
    public static void InicioSesion() {
        ...
        while (true) {
            ...
            if (SelectUser == 0) {
                System.exit(0);
            } else if (SelectUser < 1 || SelectUser > NameUser.size() + 1) {
        ...
        int intentos = 3;
        while (true) {
            System.out.println("-Si desea volver atras ingrese \"a\".");
            ...
            if (!correo.equals(EmailRegister.get(SelectUser - 1)) || !contra.equals(ContraRegister.get(SelectUser - 1))) {
                ...
                continue;
            } else if (!Admin&&correo.equals(EmailRegister.get(0)) && contra.equals(ContraRegister.get(0))) {
                Admin = true;
                System.out.println("Admin activado");
            } else {
    ...
    public static void CreaCuenta() {
        ...
        while (true) {
            ...
            newContr = scan.next();
            if (newContr.length() < 8) {
                System.out.println("La contraseña debe tener minimo 8 digigtos.");
                continue;
            }
            break;
        }
        ...
        while (true) {
            ...
            switch (opcAdmin) {
                ...
                case 5:
                    return;
                default:
                    System.out.println("Ingrese la cpcion correcta");
            }
        }
    }
        ...
        while (true) {
            ...
            switch (opcAdd) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    System.out.println("=============== Cuidados del Cabello ===============");
                    System.out.print("Ingrese el nombre del producto: ");
                ...
                case 5:
                    return;
                default:
                    System.out.println("Opocion no válida.");
            }
        }
    }
    ...
    public static boolean DetalleBuy() {
        ...
        if (!RecibirDetalles) {
            System.out.println("======== No realizastes ni una compra. =======");
            return true;
        }
        System.out.println("————————┬———————————————————————————————————┬——————————");
    ...
    public static double Efectivo(double Total){
        while(true) {
            ...
            System.out.printf("El total a pagar es de S/ %.3f.",Total);
    ...
    public static void BoletaFinal(double Total,double monto){
        ...
        System.out.println("la boleta de compra se imprimio en el block de notas.");
        try(FileWriter Impri = new FileWriter("D:\\Boleta_InkaFarma.txt")) {
            Impri.write("=======================================================\n");
            Impri.write("                                                       \n");
            Impri.write("                     BOLETA DE COMPRA                  \n");
            Impri.write("                                                       \n");
            Impri.write("=======================================================\n");
            Impri.write(String.format("Usuario: %-22s  N°Boleta: %-12d%n", NameUser.get(SelectUser - 1),numBolet()));
            Impri.write(String.format("Fecha: %-17s         Hora: %-10s      %n", FechaActual(), HoraActual()));
            Impri.write("Farmacia: InkaFarma, Juliaca                           \n");
            Impri.write("________________________________________________________\n");
            Impri.write("Cantidad│             Productos             │  Precio  \n");
            Impri.write("________________________________________________________\n");
            for (int i = 0; i < CuidaRespiratorio.size(); i++) {
                if (CarriKewin.get(i) > 0) {
                    double SubKewin = CarriKewin.get(i) * PreRespi.get(i);
                    Impri.write(String.format("   %-5d│ %-34s|  %-8.2f%n", CarriKewin.get(i), CuidaRespiratorio.get(i), SubKewin));
                }
            }
            for (int i = 0; i < Vitaminas.size(); i++) {
                if (CarriAlan.get(i) > 0) {
                    double SubAlan = CarriAlan.get(i) * PreVita.get(i);
                    Impri.write(String.format("   %-5d│ %-34s|  %-8.2f%n", CarriAlan.get(i), Vitaminas.get(i), SubAlan));
                }
            }
            for (int i = 0; i < Dermatologia.size(); i++) {
                if (CarriJohan.get(i) > 0) {
                    double SubJohan = CarriJohan.get(i) * PreDerma.get(i);
                    Impri.write(String.format("   %-5d│ %-34s|  %-8.2f%n", CarriJohan.get(i), Dermatologia.get(i), SubJohan));
                }
            }
            for (int i = 0; i < CuidaCabello.size(); i++) {
                if (CarriShiron.get(i) > 0) {
                    double SubShiro = CarriShiron.get(i) * PreCabello.get(i);
                    Impri.write(String.format("   %-5d│ %-34s|  %-8.2f%n", CarriShiron.get(i), CuidaCabello.get(i), SubShiro));
                }
            }

            Impri.write("_______________________________________________________\n");
            Impri.write(String.format("               TOTAL A PAGAR                 %-9.3f\n",Total));
            Impri.write("_______________________________________________________  \n");
            Impri.write(String.format(" IGV: %-15.3f       SubNeto: %-10.3f        %n", igv, subNet));
            Impri.write(String.format(" SubNeto: %-11.3f       Vuelto: %-7.3f            %n", subNet, vuelto));
            Impri.write("_______________________________________________________\n");
            Impri.write("              MUCHAS GRACIAS POR SU COMPRA             \n");
            Impri.write("_______________________________________________________");
        }catch (IOException e){
